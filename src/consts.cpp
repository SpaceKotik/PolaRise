#include "game.hpp"
extern const int scale = 30;
extern const int field_x = 40;
extern const int field_y = 30;
extern const float heroRadius = 10;
extern const float heroAcceleration = 0.5;
extern const float maxVelocity = 5.5;
extern const float viewAngle = 70;
//extern const Color tileDefaultColor = Color(100, 100, 100);
extern const Color tileDefaultColor = Color::White;
extern const Color heroColor = Color::White;
//extern const Color backgorundColor = Color (50, 50, 50);
extern const Color backgorundColor = Color (0, 0, 0);

//extern const Color lightColorRed = Color(50, 10, 10);		//Red for 5 sources
//extern const Color lightColorBlue = Color(20, 20, 50);		//Blue for 5 sources

extern const Color lightColorBlue = Color(102, 102, 255);	//Blue for 1 source
extern const Color lightColorRed = Color(255, 51, 51);		//Red for 1 source